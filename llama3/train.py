...
def suppress_warnings_for_non_master(rank):
    if rank != 0:
        # Suppress all warnings and errors for non-master ranks
        warnings.filterwarnings("ignore")
        logging.set_verbosity_error()
    else:
        # Enable warnings and errors for the master rank (rank 0)
...
def main(model_args, data_args, training_args):
    ...
    suppress_warnings_for_non_master(rank)

    if rank == 0:
        # iterate over model args and print them
        print("Model Arguments:")
        for key, value in model_args.__dict__.items():
            print(f"{key}: {value}")
        print("--------------------------------------------------------\n")
        print("Data Training Arguments:")
