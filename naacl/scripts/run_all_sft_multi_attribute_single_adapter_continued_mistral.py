...
config_path = "/home2/tathagato/summarization/MACSUM/naacl/configs/multi_attribute_single_adapter_continued_sft.yaml"
...
experiment_names = [experiment for experiment in experiment_names if "mistral" in experiment]
...
def run_experiment(experiment, gpu_id):
    ...
    if DEBUG:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_single_adapter_continued_sft.py --experiment_name {experiment} --debug > {log_file} 2>&1"
    else:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_single_adapter_continued_sft.py --experiment_name {experiment} > {log_file} 2>&1"
