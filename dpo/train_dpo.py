...
from dataset import dpo_dataset,get_huggingface_dataset 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--attributes', default='length', help='comma separated list of attributes')

    args, training_args, model_config, extra_config = parser.parse_args_and_config()
    print("-----------------args-----------------")
    print(args)
    print("-----------------training_args-----------------")
    print(training_args)
    ...
    if model_config.lora_target_modules == 'all':
        model_config.lora_target_modules = ["k_proj", "q_proj", "v_proj", "up_proj", "down_proj", "gate_proj"]
    ...
    args.attributes = extra_config.attributes.split(",")
    model_type = "llama3.1"
    if "mistral" in model_config.model_name_or_path:
        model_type = "mistral"
    print(f"{model_type} model type and {args.attributes} attributes")
    train_dataset = dpo_dataset(attributes = args.attributes, tokenizer=tokenizer, mode = "train", model_type = model_type, size = -1)
    ...
    with save_context:
        ...
        print("Model saved in ", training_args.output_dir)
