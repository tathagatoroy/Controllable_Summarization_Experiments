...
if __name__=="__main__":
    #dual checkpoint 
    checkpoint_folder = "/scratch/tathagato/new_adapter_experiments/length_then_topic/"
    ...
    new_model = AutoModelForCausalLM.from_pretrained(base_model_path,trust_remote_code = True, 
                                            quantization_config = nf4_config, 
                                            torch_dtype = torch.float16,
                                            device_map = "cuda:0")
    print("loaded second model")
    res = print(compare_model_weights(model, new_model))
    if res:
