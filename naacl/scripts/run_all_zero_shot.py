...
config_path = "/home2/tathagato/summarization/MACSUM/naacl/configs/zero_shot.yaml"
...
def run_experiment(experiment, gpu_id):
    ...
    if DEBUG:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python zero_shot.py --experiment_name {experiment} --debug > {log_file} 2>&1"
    else:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python zero_shot.py --experiment_name {experiment} > {log_file} 2>&1"
