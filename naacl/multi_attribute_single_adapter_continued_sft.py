...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", type=str, default="/home2/tathagato/summarization/MACSUM/naacl/configs/multi_attribute_single_adapter_continued_sft.yaml")
    ...
    adapter_name = f"{old_attribute}_then_{new_attribute}"
    ...
    model = load_peft_checkpoint(config, bnb_config, config['checkpoint_path'], is_trainable= True, adapter_name= adapter_name)
    ...
    train(model, tokenizer, train_dataset, val_dataset, config, device=0, save_pretrained= True, do_wandb=config['do_wandb'], adapter_name= adapter_name)
